...
const ADMIN_2FA        = '000000'; // ← Change this to a secure 6-digit code in production
...
async function handleLogin(b: Record<string,string>) {
  ...
  if (user.role === 'admin') {
    ...
    if (twofa !== ADMIN_2FA) return fail('Invalid 2FA code');
  }
...
// ── TRANSACTIONS ─────────────────────────────────────────────────────────
// NOTE: subtotal_cad and total_cad are GENERATED ALWAYS columns — never include in INSERT/UPDATE
...
// ── SWAP (atomic double-entry: SWAP_OUT + SWAP_IN) ────────────────────
async function handleAddSwap(b: Record<string,unknown>, token?: string) {
  ...
  // SWAP_OUT — disposition leg (subtotal_cad/total_cad are GENERATED — do NOT include)
  ...
  // SWAP_IN — acquisition leg (subtotal_cad/total_cad are GENERATED — do NOT include)
...
// ── INTERNAL TRANSFER (atomic double-entry: TRANSFER_OUT + TRANSFER_IN) ───
async function handleAddTransfer(b: Record<string,unknown>, token?: string) {
  ...
  if (from_provider_id === to_provider_id) return fail('Source and destination providers must be different');
  const transfer_group_id = crypto.randomUUID();
  const d = db();
  // subtotal_cad and total_cad are GENERATED — omit from INSERT
  const { data: outTx, error: outErr } = await d.from('cs_transactions').insert({
